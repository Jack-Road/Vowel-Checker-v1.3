...
int vowelCheck(int inputTotal, string inputSentence){  //vowelCheck function, checks user input string and identifies each character for count, and keeps count of vowels, total characters, and NON-ALPHANUMERIC text separately
    int resultVowel, checkPosition;
    resultVowel = 0;  //resultVowel to contain the total number of vowels within user input string
    nonalphCount = 0;  //nonalphCount to contain the total number of NON-ALPHANUMERIC
    ...
    while(checkPosition < inputTotal){  //checkPosition is an integer which holds the position of our character identifier(checkTicker) within the user content string
        ...
        if(checkTicker == ' ' || checkTicker == ',' || checkTicker == '.' || checkTicker == '\''){  //iterates nonalphCount
            nonalphCount++;
        }
        checkPosition++;
    }
...
int main()
{
    ...
    string phraseResultP2 = " valid ALPHANUMERIC character(s), ";
    ...
    while(!kill){  //"While Loop" to execute vowel check operation multiple times in one program execution
        ...
        int resultVowel;
        resultVowel = vowelCheck(inputTotal, inputSentence);  //calls on the "vowelCheck" function to count vowels in content string
